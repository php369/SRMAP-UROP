...
/**
 * POST /api/groups/:id/update-projects
 * Update selected projects (leader only, before submission)
 * Accessible by: students (group leaders only)
 */
router.post('/:id/update-projects', authenticate, authorize('student'), async (req, res) => {
  try {
    const { id } = req.params;
    ...
    if (!selectedProjects || !Array.isArray(selectedProjects)) {
      res.status(400).json({
        ...
        error: {
          ...
          message: 'selectedProjects must be an array',
          timestamp: new Date().toISOString(),
        },
      });
      return;
    ...
    if (group.leaderId.toString() !== req.user!.id) {
      res.status(403).json({
        ...
        error: {
          ...
          message: 'Only the group leader can update selected projects',
          timestamp: new Date().toISOString(),
        },
      });
      return;
    ...
    group.selectedProjects = selectedProjects.map((id: string) => new mongoose.Types.ObjectId(id));
    await group.save();

    logger.info(`Group ${group.groupId} projects updated by leader ${req.user!.id}`);
    ...
    logger.error('Error updating selected projects:', error);
    res.status(500).json({
      success: false,
      error: {
        code: 'UPDATE_PROJECTS_FAILED',
        message: 'Failed to update selected projects',
        timestamp: new Date().toISOString(),
      },
    });
  }
...
/**
 * POST /api/groups/:id/submit-details
 * Submit member details (department, specialization)
 * Accessible by: students (group members only)
 */
router.post('/:id/submit-details', authenticate, authorize('student'), async (req, res) => {
  try {
    const { id } = req.params;
    ...
    // Initialize memberDetails if it doesn't exist
    if (!group.memberDetails) {
      group.memberDetails = [];
    }

    // Check if member already submitted details
    const existingIndex = group.memberDetails.findIndex(
      (md: any) => md.userId.toString() === req.user!.id
    );
    ...
    if (existingIndex >= 0) {
      ...
      group.memberDetails[existingIndex] = memberDetail;
    } else {
      // Add new details
      group.memberDetails.push(memberDetail);
    }

    await group.save();

    logger.info(`Member ${req.user!.id} submitted details for group ${group.groupId}`);
