...
router.post('/api/v1/auth/google', asyncHandler(async (req: Request, res: Response) => {
  // This handles the duplicated /api/v1/api/v1/auth/google path
  // Redirect to the correct handler logic
  ...
  try {
    ...
    if (!user) {
      ...
      user = new User({
        ...
        email: googleUser.email,
        avatar: googleUser.avatarUrl,
        role: authResult.role === 'coordinator' ? 'faculty' : authResult.role,
        profile: {
